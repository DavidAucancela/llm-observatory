...
│   ├── providerRegistry.js Maps constants/providers.js's capabilities to the functions that implement them (PROVIDER_BILLING for reconciliation, SYNC_PROVIDERS for sync), replacing the `provider === 'anthropic' ? … : fetchOpenAI…` ternaries that had grown dangerous as providers were added — with 5 providers the `else` branch silently meant "OpenAI", so a Grok/Gemini/Kimi admin key was sent to api.openai.com. Adding a provider's billing here is one entry; gemini/kimi are absent on purpose (no org-level billing API to call).
...
├── utils/
│   └── dateRange.js    getRangeIntervals(range) — range → SQL INTERVAL strings, shared by metrics.js summary and services/insights.js. appendTimeWindow(params, {range, start, end}, column) — the shared "range preset OR custom start/end" WHERE-clause builder for the simpler range-only endpoints (tag-keys/tag-values/tag-breakdown/project-breakdown, balances.js); custom start/end always wins over the preset when both are present. GET /api/metrics and /summary predate this helper and keep their own inline version (extra previous-period logic makes sharing it not worth the risk).
└── jobs/
...
**Query params for GET /api/metrics:** `page`, `limit`, `range` (24h|7d|30d|60d|90d|custom), `sortBy`, `sortDir`, `model`, `provider`, `start`, `end`. `start`/`end` (ISO timestamps) always override `range` when both are present — this is the convention every range-aware endpoint follows (`/summary`, `/export`, `tag-keys`, `tag-values`, `tag-breakdown`, `project-breakdown`, `/api/balances`), so a frontend custom date-range picker only ever needs to add `start`/`end` to the existing query string, never a special `range` value of its own (the web sends `range=custom` alongside them by convention, but the API ignores `range` entirely once `start`/`end` are both set). The picker's UI only offers 24h/7d/30d + a custom popover — `60d`/`90d` remain valid values for direct API callers (e.g. `/api/insights/summary`) but no longer have a button.
